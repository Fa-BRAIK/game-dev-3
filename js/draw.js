...
export const drawMap = (ctx, callback) => {
    const tiles = []
...
const draw = (ctx, tiles, mapArray) => {
    ...
    for (let i = 0; i < level2Tiles.length; i++) 
        ctx.drawImage(tiles[tile_id], 
            (level2Tiles[i].x - level2Tiles[i].y) * tileWidth / 2 + mapX, 
            (level2Tiles[i].x + level2Tiles[i].y) * tileWidth / 4 + mapY, 
            tileWidth, tileHeight);
    ...
    for (let i = 0; i < level3Tiles.length; i++)
        ctx.drawImage(tiles[tile_id], 
            (level3Tiles[i].x - level3Tiles[i].y) * tileWidth / 2 + mapX, 
            (level3Tiles[i].x + level3Tiles[i].y) * tileWidth / 4 + mapY, 
            tileWidth, tileHeight);
}